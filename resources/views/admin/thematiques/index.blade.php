...
@section('content')
    <div class="container" style="margin-top:75px;">
        ...
        @endif
            <a href="{{ route('thematiques.create') }}" id="newpage-cta" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            ...
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b">
                        <td class="px-6 py-4">
                            <a href="{{ route('thematiques.edit', ['thematique' => $thematique->id]) }}" class="underline">{{ $thematique->name }}</a>
                        </td>
                        <td class="px-6 py-4">{{ $thematique->desc }}</td>
                        <td class="px-6 py-4">
                            <div class="relative inline-block text-left dropdownHover">
                                ...
                                <div class="absolute right-0 z-10 my-0 w-56 origin-top-right rounded-md bg-white shadow-lg focus:outline-none h-0 overflow-hidden dropdown-child dropdownHover" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="flex flex-col dropdownHover" role="none">
                                        <a href="{{ route('thematiques.edit', ['thematique' => $thematique->id]) }}" class="hover:bg-gray-200 py-1 px-4 dropdownHover">Éditer</a>
                                        <form action="{{ route('thematiques.destroy', ['thematique' => $thematique->id]) }}" method="POST">
                                            @csrf
                                            {{method_field('DELETE')}}
                                            <input type="submit" value="Supprimer" class="hover:bg-gray-200 py-1 px-4 dropdownHover cursor-pointer text-left w-full" onclick="return confirm('Êtes-vous sure de vouloir supprimer?')"> 
                                        </form>
                                    </div>
                                </div>
                            </div>
